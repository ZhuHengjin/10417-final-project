...
from crd_sw.criterion import CRDLoss as SWCRDLoss
...
def parse_option():
    ...
    parser.add_argument('--distill', type=str, default='kd', choices=['kd', 'hint', 'attention', 'similarity',
                                                                      'correlation', 'vid', 'crd', 'crd_sw', 'crd-sw',
                                                                      'kdsvd', 'fsp',
    ...
    opt.model_name = 'S:{}_T:{}_{}_{}_r:{}_a:{}_b:{}_sw:{}_tau:{}_{}'\
        .format(opt.model_s, opt.model_t, opt.dataset, opt.distill,
                opt.gamma, opt.alpha, opt.beta, opt.sw_alpha, 
                getattr(opt, 'sw_tau', opt.nce_t), opt.trial)
...
def main():
    ...
    elif opt.distill == 'crd_sw':
        opt.s_dim = feat_s[-1].shape[1]
        opt.t_dim = feat_t[-1].shape[1]
        opt.n_data = n_data
        criterion_kd = SWCRDLoss(opt)
        module_list.append(criterion_kd.embed_s)
        module_list.append(criterion_kd.embed_t)
        trainable_list.append(criterion_kd.embed_s)
        trainable_list.append(criterion_kd.embed_t)
