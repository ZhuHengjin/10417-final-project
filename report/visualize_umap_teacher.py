#!/usr/bin/env python3
"""Generate a UMAP plot from a trained teacher checkpoint."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="UMAP visualization for a teacher checkpoint")
    parser.add_argument(
        "--ckpt",
        required=True,
        help="Path to the teacher checkpoint (e.g., save/models/resnet110_cifar100_lr_0.05_decay_0.0005_trial_0/resnet110_best.pth)",
    )
    parser.add_argument("--model", required=True, choices=model_dict.keys(), help="Teacher architecture name")
    ...
    parser.add_argument("--out", default="umap_teacher.png", help="Output path for the figure")
...
def load_teacher(model_name: str, ckpt_path: str, device: str) -> torch.nn.Module:
...
def collect_features(
    ...
    with torch.no_grad():
        for images, targets in loader:
            ...
            feat_list, _ = model(images, is_feat=True)
            feat = feat_list[-1]
            if feat.dim() > 2:
...
def plot_embedding(embedding: np.ndarray, labels: np.ndarray, out_path: str) -> None:
    ...
    plt.xlabel("UMAP-1")
    plt.ylabel("UMAP-2")
    plt.title("Teacher features (final hidden state)")
...
def main() -> None:
    ...
    model = load_teacher(args.model, args.ckpt, args.device)
